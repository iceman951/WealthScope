 ...
 *
 * Builds one clearly fictional household — accounts, assets in two currencies,
...
/**
 * Inserts the demo household. Existing rows are left alone unless `reset` is set,
 * so calling this on an already-seeded database is a no-op.
 *
 * `today` is resolved per call rather than at module load: a dev server can stay
 * up for days, and a stale date would put the "recent" history in the past.
 */
export async function seedDemoData(db: DbClient, options: { reset?: boolean } = {}): Promise<void> {
